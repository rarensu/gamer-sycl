...
void CUSRC_SrcSolver_IterateAllCells(
    ...
    /*
    DPCT1102:33: Zero-length arrays are not permitted in SYCL device code.
    */
    const real g_Mag_Array_In[][NCOMP_MAG][SRC_NXT_P1 * SQR(SRC_NXT)],
    const double g_Corner_Array[][3], const SrcTerms_t SrcTerms,
    const int NPatchGroup, const real dt, const real dh, const double TimeNew,
    const double TimeOld, const real MinDens, const real MinPres,
    const real MinEint, const long PassiveFloor, const EoS_t EoS);
...
#else
/*
DPCT1102:34: Zero-length arrays are not permitted in SYCL device code.
*/
...
//=========================================================================================
   for (int s=0; s<GPU_NStream; s++)
   {
      ...
      /*
      DPCT1124:35: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
...
#     endif
      ...
      /*
      DPCT1124:36: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
      the origin API might be synchronous, it depends on the type of operand
      memory, so you may need to call wait() on event return by memcpy API to
      ensure synchronization behavior.
      */
...
//=========================================================================================
   for (int s=0; s<GPU_NStream; s++)
   {
      ...
      */
      {
         dpct::has_capability_or_fail(Stream[s]->get_device(),
                                      {sycl::aspect::fp64});

         Stream[s]->submit([&](sycl::handler &cgh) {
            const float (*)[5][512] d_Flu_Array_S_In_UsedPatch_s_ct0 =
                d_Flu_Array_S_In + UsedPatch[s];
            auto d_Flu_Array_S_Out_UsedPatch_s_ct1 =
                d_Flu_Array_S_Out + UsedPatch[s];
            const float (*)[0][576] d_Mag_Array_S_In_UsedPatch_s_ct2 =
                d_Mag_Array_S_In + UsedPatch[s];
            const double (*)[3] d_Corner_Array_S_UsedPatch_s_ct3 =
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1) {
                   CUSRC_SrcSolver_IterateAllCells(
                       ...
                       d_Corner_Array_S_UsedPatch_s_ct3, SrcTerms, NPatchGroup,
                       dt, dh, TimeNew, TimeOld, MinDens, MinPres, MinEint,
                       PassiveFloor, EoS);
                });
         });
      }
...
//=========================================================================================
   for (int s=0; s<GPU_NStream; s++)
   {
      ...
      /*
      DPCT1124:38: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
      the origin API might be synchronous, it depends on the type of operand
      memory, so you may need to call wait() on event return by memcpy API to
      ensure synchronization behavior.
      */
