...
void FLU_FluidSolver_CTU(
    ...
    real g_Flu_Array_Out[][NCOMP_TOTAL][CUBE(PS2)],
    /*
    DPCT1102:21: Zero-length arrays are not permitted in SYCL device code.
    */
    const real g_Mag_Array_In[][NCOMP_MAG][FLU_NXT_P1 * SQR(FLU_NXT)],
    /*
    DPCT1102:22: Zero-length arrays are not permitted in SYCL device code.
    */
    real g_Mag_Array_Out[][NCOMP_MAG][PS2P1 * SQR(PS2)],
    char g_DE_Array_Out[][CUBE(PS2)],
    real g_Flux_Array[][9][NCOMP_TOTAL][SQR(PS2)],
    ...
    real g_FC_Flux[][3][NCOMP_TOTAL_PLUS_MAG][CUBE(N_FC_FLUX)],
    /*
    DPCT1102:23: Zero-length arrays are not permitted in SYCL device code.
    */
    real g_FC_Mag_Half[][NCOMP_MAG][FLU_NXT_P1 * SQR(FLU_NXT)],
    /*
    DPCT1102:24: Zero-length arrays are not permitted in SYCL device code.
    */
    real g_EC_Ele[][NCOMP_MAG][CUBE(N_EC_ELE)], const real dt, const real dh,
    const bool StoreFlux, const bool StoreElectric,
    ...
    const bool NormPassive, const int NNorm, const bool FracPassive,
...
#else
/*
DPCT1102:25: Zero-length arrays are not permitted in SYCL device code.
*/
static real (*d_Mag_Array_F_In)[NCOMP_MAG][FLU_NXT_P1 * SQR(FLU_NXT)] = NULL;
/*
DPCT1102:26: Zero-length arrays are not permitted in SYCL device code.
*/
...
#else
/*
DPCT1102:27: Zero-length arrays are not permitted in SYCL device code.
*/
static real (*d_FC_Mag_Half)[NCOMP_MAG][FLU_NXT_P1 * SQR(FLU_NXT)] = NULL;
/*
DPCT1102:28: Zero-length arrays are not permitted in SYCL device code.
*/
...
#  if (  !( MODEL == ELBDM  &&  WAVE_SCHEME == WAVE_GRAMFE  &&  GRAMFE_SCHEME == GRAMFE_FFT )  )
   const dpct::dim3 BlockDim_FluidSolver(FLU_BLOCK_SIZE_X, FLU_BLOCK_SIZE_Y,
                                         1); // for the fluid solvers
#  endif
#  if ( ELBDM_SCHEME == ELBDM_HYBRID )
...
#     endif
      /*
      DPCT1124:29: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
...
#        elif ( FLU_SCHEME == CTU )
         ...
         */
      {
         ...
         Stream[s]->submit([&](sycl::handler &cgh) {
            auto c_NormIdx_ptr_ct1 = c_NormIdx.get_ptr();
            auto c_FracIdx_ptr_ct1 = c_FracIdx.get_ptr();

            const float (*)[5][10648] d_Flu_Array_F_In_UsedPatch_s_ct0 =
                d_Flu_Array_F_In + UsedPatch[s];
            auto d_Flu_Array_F_Out_UsedPatch_s_ct1 =
                d_Flu_Array_F_Out + UsedPatch[s];
            const float (*)[0][11132] d_Mag_Array_F_In_UsedPatch_s_ct2 =
                d_Mag_Array_F_In + UsedPatch[s];
            auto d_Mag_Array_F_Out_UsedPatch_s_ct3 =
                d_Mag_Array_F_Out + UsedPatch[s];
            auto d_DE_Array_F_Out_UsedPatch_s_ct4 =
                d_DE_Array_F_Out + UsedPatch[s];
            auto d_Flux_Array_UsedPatch_s_ct5 = d_Flux_Array + UsedPatch[s];
            auto d_Ele_Array_UsedPatch_s_ct6 = d_Ele_Array + UsedPatch[s];
            const double (*)[3] d_Corner_Array_F_UsedPatch_s_ct7 =
                d_Corner_Array_F + UsedPatch[s];
            const float (*)[1] d_Pot_Array_USG_F_UsedPatch_s_ct8 =
                d_Pot_Array_USG_F + UsedPatch[s];
            auto d_PriVar_UsedPatch_s_ct9 = d_PriVar + UsedPatch[s];
            auto d_Slope_PPM_UsedPatch_s_ct10 = d_Slope_PPM + UsedPatch[s];
            auto d_FC_Var_UsedPatch_s_ct11 = d_FC_Var + UsedPatch[s];
            auto d_FC_Flux_UsedPatch_s_ct12 = d_FC_Flux + UsedPatch[s];
            auto d_FC_Mag_Half_UsedPatch_s_ct13 = d_FC_Mag_Half + UsedPatch[s];
            auto d_EC_Ele_UsedPatch_s_ct14 = d_EC_Ele + UsedPatch[s];
            auto GPUExtAcc_Ptr_ct24 = GPUExtAcc_Ptr;
            ...
            cgh.parallel_for(
                ...
                [=](sycl::nd_item<3> item_ct1) {
                   FLU_FluidSolver_CTU(
                       ...
                       d_Mag_Array_F_Out_UsedPatch_s_ct3,
                       d_DE_Array_F_Out_UsedPatch_s_ct4,
                       d_Flux_Array_UsedPatch_s_ct5,
                       d_Ele_Array_UsedPatch_s_ct6,
                       d_Corner_Array_F_UsedPatch_s_ct7,
                       d_Pot_Array_USG_F_UsedPatch_s_ct8,
                       d_PriVar_UsedPatch_s_ct9, d_Slope_PPM_UsedPatch_s_ct10,
                       d_FC_Var_UsedPatch_s_ct11, d_FC_Flux_UsedPatch_s_ct12,
                       d_FC_Mag_Half_UsedPatch_s_ct13,
                       d_EC_Ele_UsedPatch_s_ct14, dt, dh, StoreFlux,
                       StoreElectric, LR_Limiter, MinMod_Coeff, Time, UsePot,
                       ExtAcc, GPUExtAcc_Ptr_ct24, MinDens, MinPres, MinEint,
                       DualEnergySwitch, PassiveFloor, NormPassive, NNorm,
                       FracPassive, NFrac, JeansMinPres, JeansMinPres_Coeff,
                       EoS, *c_NormIdx_ptr_ct1, *c_FracIdx_ptr_ct1);
                });
         });
      }
...
#     endif // MODEL
      ...
      /*
      DPCT1010:30: SYCL uses exceptions to report errors and does not use the
...
#     endif
      /*
      DPCT1124:31: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
      the origin API might be synchronous, it depends on the type of operand
      memory, so you may need to call wait() on event return by memcpy API to
      ensure synchronization behavior.
      */
...
#     endif
      ...
      if ( StoreFlux )
      /*
      DPCT1124:32: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
      the origin API might be synchronous, it depends on the type of operand
      memory, so you may need to call wait() on event return by memcpy API to
      ensure synchronization behavior.
      */
