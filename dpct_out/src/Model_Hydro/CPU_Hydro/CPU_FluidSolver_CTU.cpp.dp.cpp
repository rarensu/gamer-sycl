...
// internal functions
GPU_DEVICE
void Hydro_TGradientCorrection(
    real g_FC_Var[][NCOMP_TOTAL_PLUS_MAG][CUBE(N_FC_VAR)],
    const real g_FC_Flux[][NCOMP_TOTAL_PLUS_MAG][CUBE(N_FC_FLUX)],
    const real g_FC_B_In[][FLU_NXT_P1 * SQR(FLU_NXT)],
    const real g_FC_B_Half[][FLU_NXT_P1 * SQR(FLU_NXT)],
    /*
    DPCT1102:110: Zero-length arrays are not permitted in SYCL device code.
    */
    const real g_EC_Ele[][CUBE(N_EC_ELE)], const real g_PriVar[][CUBE(FLU_NXT)],
...
void FLU_FluidSolver_CTU(
    ...
    /*
    DPCT1102:111: Zero-length arrays are not permitted in SYCL device code.
    */
    const real g_Mag_Array_In[][NCOMP_MAG][FLU_NXT_P1 * SQR(FLU_NXT)],
    /*
    DPCT1102:112: Zero-length arrays are not permitted in SYCL device code.
    */
    ...
    /*
    DPCT1102:113: Zero-length arrays are not permitted in SYCL device code.
    */
    real g_FC_Mag_Half[][NCOMP_MAG][FLU_NXT_P1 * SQR(FLU_NXT)],
    /*
    DPCT1102:114: Zero-length arrays are not permitted in SYCL device code.
    */
...
#        else
         ...
         /*
         DPCT1102:115: Zero-length arrays are not permitted in SYCL device code.
         */
...
Hydro_TGradientCorrection exceeds 128 bytes and may cause high register
...
void Hydro_TGradientCorrection(
    ...
    const real g_FC_B_Half[][FLU_NXT_P1 * SQR(FLU_NXT)],
    /*
    DPCT1102:116: Zero-length arrays are not permitted in SYCL device code.
    */
    const real g_EC_Ele[][CUBE(N_EC_ELE)], const real g_PriVar[][CUBE(FLU_NXT)],
    const real dt, const real dh, const real MinDens, const real MinEint,
    const long PassiveFloor)
