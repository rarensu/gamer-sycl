...
#else
/*
DPCT1064:16: Migrated fabsf call is used in a macro/template definition and may
not be valid for all macro/template uses. Adjust the code.
*/
#define FABS(a) sycl::fabs(fluid[MOMZ])
/*
DPCT1064:106: Migrated sqrtf call is used in a macro/template definition and may
not be valid for all macro/template uses. Adjust the code.
*/
#define SQRT(a) sycl::sqrt((float)a)
#  define   SIN( a )         sinf( a )
#  define   COS( a )         cosf( a )
#  define   LOG( a )         logf( a )
#  define   EXP( a )         expf( a )
...
/*
DPCT1064:15: Migrated fmaxf call is used in a macro/template definition and may
not be valid for all macro/template uses. Adjust the code.
*/
#define FMAX(a, b)                                                             \
   sycl::fmax(                                                                 \
       FMAX(FABS(cc_L), FMAX(FABS(cc_C), FMAX(FABS(cc_R), FABS(cc_RR)))),      \
       FMAX(FABS(cc_L), FMAX(FABS(cc_C), FMAX(FABS(cc_R), FABS(cc_RR)))))
/*
DPCT1064:95: Migrated fminf call is used in a macro/template definition and may
not be valid for all macro/template uses. Adjust the code.
*/
#define FMIN(a, b)                                                             \
   sycl::fmin(FMIN(C_factor *FABS(dd_C),                                       \
                   FMIN(C_factor *FABS(dd_R), (real)6.0 * FABS(ddh_C))),       \
              FMIN(C_factor *FABS(dd_C),                                       \
                   FMIN(C_factor *FABS(dd_R), (real)6.0 * FABS(ddh_C))))
#define POW(a, b) dpct::pow(a, b)
