...
// set up the simulation box
   ...
   printf( "   NX0_Tot [1] = %8d\n",     NX0_Tot[1]      );
   printf( "   NX0_Tot [2] = %8d\n",     NX0_Tot[2]      );
   printf( "   BoxSize [0] = %14.5lf\n", amr.BoxSize[0]  );
   printf( "   BoxSize [1] = %14.5lf\n", amr.BoxSize[1]  );
   printf( "   BoxSize [2] = %14.5lf\n", amr.BoxSize[2]  );
   printf( "   BoxScale[0] = %8d\n",     amr.BoxScale[0] );
   printf( "   BoxScale[1] = %8d\n",     amr.BoxScale[1] );
   printf( "   BoxScale[2] = %8d\n",     amr.BoxScale[2] );
...
// =================================================================================================
   ...
   int  CrMin[3], CrMax[3];   // corner coordinates of each patch
   bool GotYou;
...
//       e2. create the patch and load the physical data if it is a leaf patch
         if ( LoadSon == -1 )
         {
            ...
            for (int dim=0; dim<3; dim++)
            {
               CrMin[dim] = LoadCorner[dim];
               CrMax[dim] = LoadCorner[dim] + PATCH_SIZE*amr.scale[lv];
            }

            if ( CrMin[0] < CanMax[0]  &&  CrMax[0] > CanMin[0] )
            if ( CrMin[1] < CanMax[1]  &&  CrMax[1] > CanMin[1] )
            if ( CrMin[2] < CanMax[2]  &&  CrMax[2] > CanMin[2] )
               GotYou = true;
            ...
            if ( GotYou )
            {
               ...
               amr.pnew( lv, LoadCorner[0], LoadCorner[1], LoadCorner[2] );
...
//             e2-1. load the fluid variables
               if ( DataOrder_xyzv )
               {
                  ...
                  fread( amr.patch[lv][PID]->fluid, sizeof(real), PATCH_SIZE*PATCH_SIZE*PATCH_SIZE*NCOMP, File );
