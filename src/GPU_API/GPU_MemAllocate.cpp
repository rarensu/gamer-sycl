...
void GPU_SetMemSize( int &GPU_NStream, int &Flu_GPU_NPGroup, int &Pot_GPU_NPGroup, int &Che_GPU_NPGroup,
                       int &Src_GPU_NPGroup );
...
#ifdef GRAVITY
int GPU_MemAllocate_PoissonGravity( const int Pot_NPatchGroup );
...
//    set parameters related to GPU memory allocation
      GPU_SetMemSize( GPU_NSTREAM, FLU_GPU_NPGROUP, POT_GPU_NPGROUP, CHE_GPU_NPGROUP, SRC_GPU_NPGROUP );


//    allocate memory for all GPU solvers
      if ( Status == GAMER_SUCCESS )   Status = GPU_MemAllocate_Fluid( FLU_GPU_NPGROUP, POT_GPU_NPGROUP, SRC_GPU_NPGROUP, GPU_NSTREAM );
#     ifdef GRAVITY
      if ( Status == GAMER_SUCCESS )   Status = GPU_MemAllocate_PoissonGravity( POT_GPU_NPGROUP );
#     endif
...
//          set *_GPU_NPGROUP<=0 if they are not fixed manually so that they can be reset by GPU_SetMemSize() later
