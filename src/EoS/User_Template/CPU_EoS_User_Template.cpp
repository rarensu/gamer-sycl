...
#  ifdef GAMER_DEBUG
// note that some EoS may support Eint<0
   if (  Hydro_IsUnphysical_Single( Eint, "output internal energy", (real)0.0, HUGE_NUMBER, ERROR_INFO, UNPHY_VERBOSE )  )
   {
...
// check
#  ifdef GAMER_DEBUG
   if (  Hydro_IsUnphysical_Single( Cs2, "output sound speed squared", (real)0.0, HUGE_NUMBER, ERROR_INFO, UNPHY_VERBOSE )  )
   {
      printf( "Dens=%13.7e, Pres=%13.7e\n", Dens, Pres );
#     if ( NCOMP_PASSIVE > 0 )
      printf( "Passive scalars:" );
...
// check
#  ifdef GAMER_DEBUG
   if (  Hydro_IsUnphysical_Single( Temp, "output temperature", (real)0.0, HUGE_NUMBER, ERROR_INFO, UNPHY_VERBOSE )  )
   {
      printf( "Dens=%13.7e, Eint=%13.7e\n", Dens, Eint );
#     if ( NCOMP_PASSIVE > 0 )
      printf( "Passive scalars:" );
...
// check
#  ifdef GAMER_DEBUG
   if (  Hydro_IsUnphysical_Single( Entr, "output entropy", (real)0.0, HUGE_NUMBER, ERROR_INFO, UNPHY_VERBOSE )  )
   {
      printf( "Dens=%13.7e, Eint=%13.7e\n", Dens, Eint );
#     if ( NCOMP_PASSIVE > 0 )
      printf( "Passive scalars:" );
...
static dpct::global_memory<EoS_DP2E_t, 0>
    EoS_DensPres2Eint_Ptr(EoS_DensPres2Eint_User_Template);
static dpct::global_memory<EoS_DP2C_t, 0>
    EoS_DensPres2CSqr_Ptr(EoS_DensPres2CSqr_User_Template);
static dpct::global_memory<EoS_DE2T_t, 0>
    EoS_DensEint2Temp_Ptr(EoS_DensEint2Temp_User_Template);
static dpct::global_memory<EoS_DT2P_t, 0>
    EoS_DensTemp2Pres_Ptr(EoS_DensTemp2Pres_User_Template);
...
//                   --> It will be copied to GPU automatically in GPUAPI_SetConstMemory()
